...
class UserDomainServiceTest {
    ...
    fun setUp() {
        ...
        service = UserDomainService(userRepository, passwordService)
    }
    ...
    inner class CreateUser {
        @Test
        @DisplayName("중복되지 않는 이메일과 유효한 비밀번호로 사용자 생성 성공 시 UserEntity를 반환한다")
        fun `create user when email unique and password valid expect entity created`() {
            // Arrange
            val email = "user@example.com"
            val rawPassword = "Aa1!aaaa"
            val name = "Tomo"

            every { userRepository.findByEmail(email) } returns null
            every { passwordService.validate(rawPassword) } returns ValidationResult(isValid = true)
            every { passwordService.encode(rawPassword) } returns "hashed-password"
            every { userRepository.save(any<UserEntity>()) } returnsArgument 0

            // Act
            val created = service.createUser(email, rawPassword, name)
            ...
            assertThat(created.password.value).isEqualTo("hashed-password")
        ...
        fun `create user when email duplicated expect exception`() {
            ...
            val existing = UserEntity(id = 1, email = email, password = HashedPassword("x"), username = "name")
            every { userRepository.findByEmail(email) } returns existing

            // Act & Assert
            assertThatThrownBy { service.createUser(email, "Aa1!aaaa", "Name") }
        ...
        @Test
        @DisplayName("비밀번호 정책을 만족하지 않는 비밀번호로 사용자 생성 시 예외를 던진다")
        fun `create user when password policy violated expect 400 bad request`() {
            // Arrange
            val email = "user@example.com"
            val rawPassword = "short"
            every { userRepository.findByEmail(email) } returns null
            every { passwordService.validate(rawPassword) } returns ValidationResult(isValid = false, violations = listOf("len"))

            // Act & Assert
            assertThatThrownBy { service.createUser(email, rawPassword, "Name") }
